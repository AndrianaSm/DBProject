...
//                        System.out.println("[Broker " + broker.getBrokerId() + "] Received: " + input);
                    ...
                    String topic=getLineCode(pubInfo[0]);
                    broker.acceptConnection(topic, Integer.parseInt(pubInfo[1]));
                    PrintWriter outputStream = new PrintWriter(socket.getOutputStream(), true);
                    while (true) {
                        Queue<Value> messages = broker.getTopics().get(topic);
                        if (messages.isEmpty()) {
    ...
    private String getLineCode (String input){
        Hash.loadData();
